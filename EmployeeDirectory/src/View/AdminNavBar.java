...
public class AdminNavBar extends JFrame {
	...
	private void createPanels() {
	    // Create both panels with the parent frame reference
	    ...
	    panels.put("AddAdmin", new AddAdminView());
	    

	    // Add all panels to the contentPanel
	    for (Map.Entry<String, JPanel> entry : panels.entrySet()) {
	        contentPanel.add(entry.getValue(), entry.getKey());
	    }
	}
	...
	public void refreshEmployeeDashboard() {
	    ...
	    dashboard.loadEmployees(); // This will reload the employee list
	}
	public void refreshAdminDashboard() {
	    AdminDashboard dashboard = (AdminDashboard) getPanels().get("AdminDashboard");
	    dashboard.loadAdmin(); // This will reload the employee list
	}
}
